...
datapath = '../../data/20110510 Stokes analysis B1 groove/'
...
offsets = N.hstack(((theta[:, N.newaxis] + 180.0) * N.pi / 180.0,
    N.ones_like(theta[:, N.newaxis]) * 7.0))

# plot it
patch_args = {'edgecolor': tango.scarletred1, 'facecolor': 'none'}
fig = P.figure(figsize=(plot_config.textwidth, 2.0))
ax = fig.add_subplot(1, 2, 1, aspect='equal', xticks=[], yticks=[])
im = ax.imshow(St[0], vmin=3000, vmax=20000, origin='upper')
ax.add_patch(P.Circle((xc, yc), rin, **patch_args))
...
fig.savefig('groove-near-field.pdf')
...
ax.plot(angles, N.cos(2 * angles), color=tango.skyblue3, label='')
ax.plot(angles, N.sin(2 * angles), color=tango.scarletred3, label='')
...
fig.tight_layout()
fig.savefig('groove-stokes-analysis.pdf')
